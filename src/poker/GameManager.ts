...
export class GameManager {
    ...
    public async processMessage(ws: any, message: any, user: { id: number, first_name: string }) {
        try {
            ...
            if (msg.type === 'JOIN') {
                await this.joinLobby(ws, userId, user.first_name, msg.lobbyId, msg.password, msg.seatIndex, msg.avatarUrl);
                return;
            }
    ...
    private async joinLobby(ws: any, userId: number, userName: string, lobbyId: number, password?: string, seatIndex?: number, avatarUrl?: string) {
        ...
        const joined = await table.addPlayer({ id: userId, name: userName, coins: coins, avatarUrl }, broadcaster, seatIndex);
