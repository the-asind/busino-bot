    ...
    return (
        <>
            ...
            <div className="absolute bottom-0 left-0 right-0 z-[100] bg-slate-900/95 border-t border-slate-700 p-4 rounded-t-2xl shadow-2xl animate-slide-up backdrop-blur-md w-full">
                <div className="grid grid-cols-8 md:grid-cols-10 gap-2 max-h-[40vh] overflow-y-auto custom-scrollbar pb-safe">
                    {stickers.map(id => (
                        <button
                            key={id}
                            onClick={() => onSelect(id)}
                            className="aspect-square hover:bg-slate-700/50 rounded-lg transition-all active:scale-90 flex items-center justify-center p-1"
                        >
